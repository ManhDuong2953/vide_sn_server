...
const markNoticeAsRead = async (req, res) => {
  ...
  try {
    ...
    if (isMarked) {
      ...
      res
        .status(404)
        .json({ status: false, message: "Không tìm thấy thông báo" });
    }
  } catch (error) {
...
const deleteNotice = async (req, res) => {
  ...
  try {
    ...
    if (isDeleted) {
      res
        .status(200)
        .json({ status: true, message: "Thông báo đã được xoá thành công" });
    } else {
      res
        .status(404)
        .json({ status: false, message: "Không tìm thấy thông báo" });
...
const getNoticeById = async (req, res) => {
  ...
  try {
    ...
    if (notice) {
      ...
      res
        .status(404)
        .json({ status: false, message: "Thông báo không tồn tại" });
    }
  } catch (error) {
