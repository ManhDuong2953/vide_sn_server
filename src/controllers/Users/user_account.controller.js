  ...
  try {
    ...
    if (user_id) {
      ...
      new ProfileMedia({
        ...
        media_type: "cover",
        media_link:
          "https://res.cloudinary.com/der2ygna3/image/upload/v1730082713/users/cover/mzj5tslenqew6qdcpime.jpg",
      }).create();
...
const getUserById = async (req, res) => {
  try {
    if (!req.params?.id || req.params?.id === 'uid_undefined') {
...
const getAllUsers = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Danh sách người dùng", data: users });
  } catch (error) {
    console.log(error);
    res.status(500).json({ status: false, message: error.message ?? error });
  }
...
const updateUser = async (req, res) => {
  try {
    ...
    if (result > 0) {
      return res
        .status(200)
        .json({ status: true, message: "Người dùng đã được cập nhật" });
    } else {
      return res
        .status(400)
        .json({ status: false, message: "Lỗi khi cập nhật người dùng" });
    }
  } catch (error) {
    console.error("Error updating user:", error);
    return res
      .status(500)
      .json({ status: false, message: "Lỗi hệ thống khi cập nhật người dùng" });
  }
};
...
const updateUserPassword = async (req, res) => {
  try {
    ...
    if (result > 0) {
      res
        .status(200)
        .json({ status: true, message: "Mật khẩu đã được cập nhật" });
    } else {
      res
        .status(400)
        .json({ status: false, message: "Lỗi khi cập nhật mật khẩu" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ status: false, message: error.message ?? error });
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (result > 0) {
      ...
      res
        .status(400)
        .json({ status: false, message: "Lỗi khi xóa người dùng" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ status: false, message: error.message ?? error });
